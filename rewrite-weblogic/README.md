...
For a Maven project with `war` packaging, the Spring recipe creates `src/main/webapp/WEB-INF/weblogic.xml` when it is absent or merges with the existing descriptor. It adds selective application preferences for Jackson and Jakarta JAXB packages and for the Jackson module service-provider resource, preventing WebLogic's legacy `javax.xml.bind` Jackson module from being discovered by the migrated application. Existing unrelated descriptor entries are preserved. For a non-Maven build or a nonstandard web-resource layout, the recipe updates an existing `weblogic.xml` but does not create one automatically.
